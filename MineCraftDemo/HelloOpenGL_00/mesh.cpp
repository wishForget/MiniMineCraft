...
bool Mesh::InitFromScene(const aiScene* pScene, const string& Filename)
{
	...
	for (unsigned int i = 0; i < m_Entries.size(); i++) {
		...
		//mNumFaces说明了有多少个多边形,mFaces 数组包含了顶点的索引
	...
	vector<unsigned int>& Indices)
{
	...
	for (unsigned int i = 0; i < paiMesh->mNumVertices; i++) {
		const aiVector3D* pPos = &(paiMesh->mVertices[i]);  //载入顶点数据，每个顶点由x,y,z组成
	...
	// aiMesh 类的成员 mNumFaces 告诉我们有多少个多边形（一般为三角形），而 mFaces 数组包含了顶点的索引。
...
void Mesh::ReadNodeHeirarchy(float AnimationTime, const aiNode* pNode, const Matrix4f& ParentTransform)
{
	...
	//for(int i=0; i<m_pScene->mAnimations[0]->)
...
vector<Vector3f> Mesh::GetVertexs() {
	return vertexs;
}

vector<unsigned int> Mesh::GetTriangleIndexBase() {
	return triangleIndexBase;
}